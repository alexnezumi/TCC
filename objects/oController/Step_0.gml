    ...
    switch (tutorial_passo) {
        ...
        case 10: // Apenas cria o cliente e avança o passo.
            if (!tutorial_cliente_spawned) {
                var spawn_x = 950; var spawn_y = 65; // Use sua posição de spawn
...
if (!global.pause && !global.tutorial_ativo && timer_dia_frames > 0) {
    timer_dia_frames--;
    
    // <<< NOVO BLOCO: VERIFICAÇÃO DOS 3 SEGUNDOS FINAIS >>>
    // Calcula o equivalente a 3 segundos em frames
    var tres_segundos_em_frames = 3 * game_get_speed(gamespeed_fps);
    
    // Quando o timer chegar EXATAMENTE nessa marca...
    if (timer_dia_frames == tres_segundos_em_frames) {
        show_debug_message("3 SEGUNDOS RESTANTES! TODOS OS CLIENTES FICANDO BRAVOS!");
        
        // O comando 'with' aplica este código a TODAS as instâncias de oNPCgatoLaranja na sala.
        with (oNPCgatoLaranja) {
            // Afeta apenas os clientes que estão no estado de "esperando_pedido"
            if (estado == "esperando_pedido") {
                
                // Força o estado de "bravo" e "saindo"
                sprite_index = sprite_bravo;
                estado = "indo_para_saida";
                
                // Libera a cadeira que o cliente estava ocupando
